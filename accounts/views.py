...
def group_delete(request, pk):
    if request.method != 'POST':
        messages.error(request, 'Invalid request method. Deleting a group requires POST.')
        return redirect('group_list')
    ...
    try:
        ...
        with transaction.atomic():
            ...
            PendingMessage.objects.filter(student__in=students).delete()
            Mark.objects.filter(student__in=students).delete()
            try:
                from marks.models import MarkTotal
                MarkTotal.objects.filter(student__in=students).delete()
            except Exception:
                pass
            Attendance.objects.filter(student__in=students).delete()
            FeePayment.objects.filter(student_fee__student__in=students).delete()
            StudentFee.objects.filter(student__in=students).delete()
            students.delete()
            group.sections.all().delete()
            group.delete()

        messages.success(request, f'Group "{group_name}" and all related sections/students were successfully deleted.')
    except Exception as e:
        logger.exception("Failed to delete group %s", group_name)
...
def section_delete(request, pk):
    if request.method != 'POST':
        messages.error(request, 'Invalid request method. Deleting a section requires POST.')
        return redirect('group_list')
    ...
    try:
        ...
        logger.exception("Failed to delete section %s", sec_name)
