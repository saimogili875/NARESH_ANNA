...
from .models import Exam, Mark, ExamSubjectMaxMark, ExamCategory, ExamType, GroupCategoryConfig, Subject, MarksEntryLock, MarksWhatsAppSendLog, MarkTotal
...
def exam_list(request):
    ...
    categories_qs = ExamCategory.objects.all().order_by('name')
    ...
    if selected_category_id:
        if selected_category_id.isdigit():
            selected_category = categories_qs.filter(pk=selected_category_id).first()
        else:
            selected_category = categories_qs.filter(name__iexact=selected_category_id).first()

    if selected_category and active_year:
        selected_exams = Exam.objects.filter(
            academic_year=active_year, category=selected_category
        ).select_related('group', 'category', 'exam_type').order_by('-date')
...
def exam_delete(request, exam_id):
    if request.method != 'POST':
        messages.error(request, 'Invalid request method. Deleting an exam requires POST.')
        return redirect('exam_list')
    ...
    try:
        with transaction.atomic():
            exam.marks.all().delete()
            exam.subject_max_marks.all().delete()
            try:
                from marks.models import MarkTotal
                MarkTotal.objects.filter(exam=exam).delete()
            except Exception:
                pass
            exam.delete()
        messages.success(request, f"Exam '{name}' and all associated marks were successfully deleted.")
    except Exception as e:
        messages.error(request, f"Could not delete exam '{name}': {e}")
...
def marks_entry(request, exam_id):
    ...
    if request.method == 'POST':
        ...
        existing_totals = {
            mt.student_id: mt
            for mt in MarkTotal.objects.filter(exam=exam, student__section=sec)
        }
        totals_to_create = []
        totals_to_update = []
        exam_max_total = sum(subject_max_marks.values()) if subject_max_marks else exam.max_marks

        for student in Student.objects.filter(section=sec, is_active=True):
            auto_total = 0.0
            has_subject_marks = False
            for subject in subjects:
                val = request.POST.get(f'mark_{student.pk}_{subject.name}', '').strip()
                if val:
                    try:
                        ...
                        if num_f > 0:
                            auto_total += num_f
                            has_subject_marks = True
                    except (ValueError, TypeError):
                        pass

            manual_total_str = request.POST.get(f'total_mark_{student.pk}', '').strip()
            manual_total_val = None
            if manual_total_str:
                try:
                    manual_total_val = float(manual_total_str)
                except (ValueError, TypeError):
                    pass

            target_total = None
            target_source = MarkTotal.SOURCE_AUTO

            if manual_total_val is not None:
                if manual_total_val < 0 or manual_total_val > exam_max_total:
                    messages.warning(request, f"Total for {student.name} ({manual_total_val}) invalid; adjusted to bounds [0-{exam_max_total}].")
                    manual_total_val = min(max(0, manual_total_val), exam_max_total)
                target_total = round(manual_total_val, 2)
                target_source = MarkTotal.SOURCE_MANUAL
            elif has_subject_marks:
                target_total = round(auto_total, 2)
                target_source = MarkTotal.SOURCE_AUTO

            if target_total is not None:
                existing_mt = existing_totals.get(student.pk)
                if existing_mt:
                    existing_mt.total_obtained = target_total
                    existing_mt.source = target_source
                    totals_to_update.append(existing_mt)
                else:
                    totals_to_create.append(MarkTotal(
                        student=student, exam=exam, total_obtained=target_total, source=target_source
                    ))

        with transaction.atomic():
            if totals_to_create:
                MarkTotal.objects.bulk_create(totals_to_create, batch_size=200)
            if totals_to_update:
                MarkTotal.objects.bulk_update(totals_to_update, ['total_obtained', 'source'], batch_size=200)
    ...
    if selected_section:
        ...
        totals_map = {
            mt.student_id: mt
            for mt in MarkTotal.objects.filter(exam=exam, student__in=students)
        }
        exam_max_total = sum(subject_max_marks.values()) if subject_max_marks else exam.max_marks

        for student in students:
            s_marks = [(sub, marks_map.get(student.pk, {}).get(sub.id, '')) for sub in subjects]
            mt = totals_map.get(student.pk)
            
            # Subject sum for comparison
            sub_sum = 0.0
            has_sub_marks = False
            for _, mval in s_marks:
                if mval:
                    try:
                        fval = float(mval)
                        if fval > 0:
            ...
            total_val = mt.total_obtained if mt else (sub_sum if has_sub_marks else None)
            total_source = mt.source if mt else ('auto' if has_sub_marks else 'none')
            
            pct = round((float(total_val) / exam_max_total * 100), 1) if (total_val is not None and exam_max_total > 0) else None
            mismatch = (mt and mt.source == MarkTotal.SOURCE_MANUAL and has_sub_marks and round(float(mt.total_obtained), 2) != sub_sum)
