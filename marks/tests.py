...
class MarkTotalTestCase(TestCase):
    def setUp(self):
        from accounts.models import Section
        from students.models import Student
        from marks.models import MarkTotal

        self.year = AcademicYear.objects.create(name="2025-2026", is_active=True, start_date=date(2025, 6, 1), end_date=date(2026, 5, 31))
        self.group = Group.objects.create(name="MPC Total", code="MPC_MT", academic_year=self.year)
        self.section = Section.objects.create(group=self.group, year="1", name="A", academic_year=self.year)
        self.student = Student.objects.create(admission_number="ST101", name="Test Student", mobile="9876543210", section=self.section, academic_year=self.year)
        self.category = ExamCategory.objects.create(name="IPE-MPC", is_fixed_marks=False)
        self.subject1 = Subject.objects.create(name="Physics")
        self.subject2 = Subject.objects.create(name="Maths")
        self.category.subjects.add(self.subject1, self.subject2)

        self.exam_type = ExamType.objects.create(name="Mid Term")
        self.exam = Exam.objects.create(exam_type=self.exam_type, academic_year=self.year, group=self.group, category=self.category, date=date.today(), max_marks=200)

        self.admin = User.objects.create(username="admin_total", role="admin", is_superuser=True)
        self.client = Client()
        self.client.force_login(self.admin)

    def test_auto_and_manual_mark_totals(self):
        from marks.models import MarkTotal
        url = reverse('marks_entry', args=[self.exam.id])

        # 1. Post subject marks -> auto calculated total
        res = self.client.post(f"{url}?section={self.section.id}", {
            'section_id': self.section.id,
            f'mark_{self.student.pk}_{self.subject1.name}': '50',
            f'mark_{self.student.pk}_{self.subject2.name}': '40',
        })
        self.assertEqual(res.status_code, 302)

        mt = MarkTotal.objects.get(student=self.student, exam=self.exam)
        self.assertEqual(float(mt.total_obtained), 90.0)
        self.assertEqual(mt.source, 'auto')

        # 2. Post manual total -> manual total takes precedence
        res2 = self.client.post(f"{url}?section={self.section.id}", {
            'section_id': self.section.id,
            f'total_mark_{self.student.pk}': '95',
        })
        self.assertEqual(res2.status_code, 302)

        mt.refresh_from_db()
        self.assertEqual(float(mt.total_obtained), 95.0)
        self.assertEqual(mt.source, 'manual')
